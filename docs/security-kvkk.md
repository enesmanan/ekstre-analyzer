...
- [x] Orijinal PDF diske/bucket'a hiç yazılmıyor: upload ham gövde (multipart/`UploadFile` yok, Starlette geçici dosyaya yazar); Faz 1 KK-4 ve Faz 3 KK-4 grep `app/api` + `jobs.py` boş (2026-09-16, lokal; CI grep Faz 5).
...
- [x] Sızıntı testi her upload'da zorunlu; başarısızsa Gemini çağrısı yok (2026-09-16, `test_api_statements.py` `leak_detected` yolu, istemci kurulmaz).
...
- [x] Önizleme bytes'ı 10 dk sonra bellekten düşüyor (2026-09-16, TTL enjekte `preview_expired`).

## 4. Uygulama güvenliği

- [ ] HTTPS only (Cloud Run); `Strict-Transport-Security: max-age=31536000; includeSubDomains` (Faz 5).
- [x] CSP `default-src 'self'; img-src 'self' data:; style-src 'self'`; üretim build'inde inline script/style yok; konsolda ihlal yok (2026-09-16, Faz 3 KK-6).
- [x] `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: same-origin` (2026-09-16, `SecurityHeadersMiddleware`).
- [x] CORS yok (aynı origin); `CORSMiddleware` eklenmemiş (2026-09-16).
- [x] Upload: ≤ 15 MB, `%PDF-` magic byte, ≤ 60 sayfa, JavaScript / gömülü dosya / form reddi (2026-09-16, Faz 3 T03).
- [x] Şifreli PDF şifresi yalnızca istek gövdesinde; loglanmaz, DB'ye yazılmaz (2026-09-16, bellek `PdfEntry`, maske sonrası `None`).
...
- [x] Yetkilendirme: her sorgu `user_id` filtreli; başka kullanıcının kaydı `404` (2026-09-16, `test_foreign_records_are_404`).
...
- [x] Bakım bayrağı Faz 3 yazma uçlarını kapatıyor (`POST /statements`, `DELETE /statements/{id}`, `PATCH /transactions/{id}`; 2026-09-16 T03). Faz 3.5/4 uçları kendi testleriyle kapanır.
