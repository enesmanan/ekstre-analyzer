...
2. Arka plan işi (threadpool): profil tespiti, maskeleme, sızıntı testi. Sızıntı testi başarısızsa `status=mask_failed` ve iş durur.
...
│   │   ├── api/               # routers: statements, transactions, summary, categories, advice, settings, export, account, admin
