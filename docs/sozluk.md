...
| Tekrar yüklenen ekstre | `DuplicateStatement`; API `duplicate_statement` | `UNIQUE(user_id, masked_sha256)`; kolon değerini Faz 3 Gemini öncesi `sha256(orijinal)` yazar (maske bytes'ı deterministik değil). POST anında yer tutucu `pending:{uuid}` |
